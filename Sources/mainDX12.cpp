...
/**
 * DirectX12 uses Microsoft's ComPtr (SmartPtr) to automatically decrement the object counter and
 * call the interface destroy function on last reference destroy. There is no
 * device->Destroy<object>() functions, we MUST use ComPtr.
 */
#include <wrl.h>
template<typename T> using MComPtr = Microsoft::WRL::ComPtr<T>;
...
void ValidationLayersDebugCallback(D3D12_MESSAGE_CATEGORY _category,
                                   D3D12_MESSAGE_SEVERITY _severity, D3D12_MESSAGE_ID _ID,
                                   LPCSTR _description, void* _context)
{
    (void)_context;
    ...
    std::wstring dets =
        SA::StringFormat(L"ID [%1]\tCategory [%2]", static_cast<int>(_ID), categoryStr);
 ...
 * For more details on Fence/Event implementation, check Swapchain synchronization implementation
 * below.
 */
...
std::array<MComPtr<ID3D12Resource>, bufferingCount> swapchainImages{
    nullptr}; // VkImage -> ID3D12Resource
...
// === Commands === /* 0004 */
/**
 * Vulkan needs only 1 CommandPool for all frames + 1 CommandBuffer per frame. Recording commands
 * are allocated and stored in the CommandBuffer. DirectX12 doesn't have CommandPool, and split the
 * CommandBuffer into "Allocator" (1 per frame) and CommandList (current list of allocated command
 * for the frame, that can be directly reused: 1 for all frame). For more details, check
 * DirectX-Graphics-Sample: HelloFrameBuffering.cpp.
 */
...
constexpr D3D12_CLEAR_VALUE sceneDepthClearValue{
    .Format = sceneDepthFormat, .DepthStencil = {1.0f, 0}
};
...
bool SubmitBufferToGPU(MComPtr<ID3D12Resource> _gpuBuffer, uint64_t _size, const void* _data,
                       D3D12_RESOURCE_STATES _stateAfter)
{
    ...
    const HRESULT hrStagBufferCreated = device->CreateCommittedResource(
        &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_SOURCE, nullptr,
        IID_PPV_ARGS(&stagingBuffer));
    if (FAILED(hrStagBufferCreated))
    {
        SA_LOG(L"Create Staging Buffer failed!", Error, DX12,
               (L"Error code: %1", hrStagBufferCreated));
        return false;
    }
     ...
     * Better code would parallelize resources loading in staging buffer and submit only once at the
     * end to execute all GPU copies.
     */
...
bool SubmitTextureToGPU(MComPtr<ID3D12Resource> _gpuTexture,
                        const std::vector<SA::Vec2ui>& _extents, uint64_t _totalSize,
                        uint32_t _channelNum, const void* _data)
{
    // Create temp upload buffer.
    MComPtr<ID3D12Resource> stagingBuffer;
    ...
    for (UINT16 i = 0; i < resDesc.MipLevels; ++i)
    {
        const D3D12_TEXTURE_COPY_LOCATION src{
            ...
            .PlacedFootprint =
                D3D12_PLACED_SUBRESOURCE_FOOTPRINT{.Offset = offset,
...
void GenerateMipMapsCPU(SA::Vec2ui _extent, std::vector<char>& _data, uint32_t& _outMipLevels,
                        uint32_t& _outTotalSize, std::vector<SA::Vec2ui>& _outExtents,
                        uint32_t _channelNum, uint32_t _layerNum = 1u)
{
    ...
    // Generate
    {
        ...
        {
            uint64_t srcLayerOffset =
                _outExtents[i - 1].x * _outExtents[i - 1].y * _channelNum * sizeof(stbi_uc);
            uint64_t currLayerOffset =
                _outExtents[i].x * _outExtents[i].y * _channelNum * sizeof(stbi_uc);
            unsigned char* dst = src + srcLayerOffset * _layerNum;

            for (uint32_t j = 0; j < _layerNum; ++j)
            {
                bool res = stbir_resize_uint8_linear(
                    src, static_cast<int32_t>(_outExtents[i - 1].x),
                    static_cast<int32_t>(_outExtents[i - 1].y), 0, dst,
                    static_cast<int32_t>(_outExtents[i].x), static_cast<int32_t>(_outExtents[i].y),
                    0, static_cast<stbir_pixel_layout>(_channelNum));
...
int main()
{
    ...
    if (true)
    {
        ...
        // GLFW
        {
            ...
            window = glfwCreateWindow(windowSize.x, windowSize.y, "FVTDX12_DX12-Window", nullptr,
                                      nullptr);
        ...
        // Renderer
        {
            ...
            if (true)
            {
                ...
                // Validation Layers (instance-level) /* 0001-I2 */
                {
                    // Debug controller
                    {
                        ...
                        const HRESULT hrDebugInterface =
                        ...
                        {
                            SA_LOG(L"Validation layer DebugController initialization failed.",
                                   Error, DX12, (L"Error Code: %1", hrDebugInterface));
                        }
                    }
                    ...
                    // Report live objects
                    {
                        ...
                        {
                            /**
                             * Enable this to trigger breakpoints on ReportLiveObjects() and have
                             * errors in VisualStudio's output window. WARNING: ReportLiveObjects()
                             * will ONLY output in VisualStudio's output window and not exe CONSOLE.
                             */
                        ...
                        {
                            SA_LOG(L"Validation layer DebugInfoQueue uninitialized failed.", Error,
                                   DX12);
                        }
                    }
                }
            ...
            if (true)
            {
                ...
                // Select first prefered GPU, listed by HIGH_PERFORMANCE. No need to manually sort
                // GPU like Vulkan.
                const HRESULT hrQueryGPU = factory->EnumAdapterByGpuPreference(
                ...
                // Validation Layers (device-level) /* 0002-1 */
                {
                    ...
                    const HRESULT hrQueryInfoQueue =
                        device->QueryInterface(IID_PPV_ARGS(&infoQueue));
                    if (SUCCEEDED(hrQueryInfoQueue))
                    {
                        /**
                         * Cookie must be provided to properly register message callback (and
                         * unregister later). Set nullptr as cookie will not crash (and no error)
                         * but won't work.
                         */
                    ...
                    {
                        SA_LOG(L"Device query info queue to enable validation layers failed.",
                               Error, DX12, (L"Error Code: %1", hrQueryInfoQueue));
                    }
                }
#endif
                ...
                // Queue /* 0002-I2 */
                {
                    ...
                    // GFX
                    {
                         ...
                         * No need to specify in advance how many queues will be used by the device
                         * object.
                         */
                        ...
                        if (FAILED(hrGFXCmdQueueCreated))
                        {
                            SA_LOG(L"Create Graphics Queue failed!", Error, DX12,
                                   (L"Error Code: %1", hrGFXCmdQueueCreated));
                            return EXIT_FAILURE;
                        }
                        else
                        {
                            ...
                            SA_LOG(L"Create Graphics Queue success.", Info, DX12,
                                   (L"\"%1\" [%2]", name, graphicsQueue.Get()));
                        }
                    }
                }
                ...
                // Synchronization /* 0002-I3 */
                {
                    ...
                    const HRESULT hrDeviceFenceCreated =
                        device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&deviceFence));
                    if (FAILED(hrDeviceFenceCreated))
                    {
                        SA_LOG(L"Create Device Fence failed!", Error, DX12,
                               (L"Error Code: %1", hrDeviceFenceCreated));
                        return EXIT_FAILURE;
                    }
                    else
                    {
                        ...
                        SA_LOG(L"Create Swapchain Fence success.", Info, DX12,
                               (L"\"%1\" [%2]", name, deviceFence.Get()));
                    }
                }
            }
            ...
            if (true)
            {
                ...
                const HRESULT hrSwapChainCreated =
                    factory->CreateSwapChainForHwnd(graphicsQueue.Get(), glfwGetWin32Window(window),
                                                    &desc, nullptr, nullptr, &swapchain1);
                if (FAILED(hrSwapChainCreated))
                {
                    SA_LOG(L"Create Swapchain failed!", Error, DX12,
                           (L"Error Code: %1", hrSwapChainCreated));
                    return EXIT_FAILURE;
                }
                else
                {
                ...
                if (FAILED(hrSwapChainCast))
                {
                    SA_LOG(L"Swapchain cast failed!", Error, DX12,
                           (L"Error Code: %1", hrSwapChainCast));
                    return EXIT_FAILURE;
                }
                ...
                {
                    const HRESULT hrSwapChainGetBuffer =
                    ...
                    if (hrSwapChainGetBuffer)
                    {
                        SA_LOG((L"Get Swapchain Buffer [%1] failed!", i), Error, DX12,
                               (L"Error Code: %1", hrSwapChainGetBuffer));
                        return EXIT_FAILURE;
                    }
                    else
                    {
                        const std::wstring name =
                            L"SwapchainBackBuffer [" + std::to_wstring(i) + L"]";
                        swapchainImages[i]->SetName(name.data());

                        SA_LOG((L"Get Swapchain Buffer [%1] success.", i), Info, DX12,
                               (L"\"%1\" [%2]", name, swapchainImages[i].Get()));
                    }
                }
                ...
                // Synchronization /* 0003.1-I */
                {
                    ...
                    const HRESULT hrSwapChainFenceCreated = device->CreateFence(
                        0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&swapchainFence));
                    if (FAILED(hrSwapChainFenceCreated))
                    {
                        SA_LOG(L"Create Swapchain Fence failed!", Error, DX12,
                               (L"Error Code: %1", hrSwapChainFenceCreated));
                        return EXIT_FAILURE;
                    }
                    else
                    {
            ...
            if (true)
            {
                ...
                for (uint32_t i = 0; i < bufferingCount; ++i)
                {
                    ...
                    const HRESULT hrCmdAllocCreated = device->CreateCommandAllocator(
                    ...
                    if (FAILED(hrCmdAllocCreated))
                    {
                        SA_LOG((L"Create Command Allocator [%1] failed!", i), Error, DX12,
                               (L"Error Code: %1", hrCmdAllocCreated));
                        return EXIT_FAILURE;
                    }
                    else
                    {
                        ...
                        SA_LOG((L"Create Command Allocator [%1] success", i), Info, DX12,
                               (L"\"%1\" [%2]", name, cmdAlloc.Get()));
                    }
                }

                // List /* 0004-I2-2 */
                {
                    const HRESULT hrCmdListCreated = device->CreateCommandList(
                        0, D3D12_COMMAND_LIST_TYPE_DIRECT, cmdAllocs[0].Get(), nullptr,
                        IID_PPV_ARGS(&cmdList));
                    if (FAILED(hrCmdListCreated))
                    {
                        SA_LOG("Create Command List failed!", Error, DX12,
                               (L"Error Code: %1", hrCmdListCreated));
                        return EXIT_FAILURE;
                    }
                    else
                    {
                        ...
                        SA_LOG(L"Create Command List success.", Info, DX12,
                               (L"\"%1\" [%2]", name, cmdList.Get()));
                    }
                }
            ...
            if (true)
            {
                // Color RT View Heap /* 0006-I1 */
                {
                    ...
                    const HRESULT hrCreateHeap =
                        device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&sceneRTViewHeap));
                    if (FAILED(hrCreateHeap))
                    {
                        SA_LOG(L"Create Color RenderTarget ViewHeap failed!", Error, DX12,
                               (L"Error Code: %1", hrCreateHeap));
                        return EXIT_FAILURE;
                    }
                    else
                    {
                        ...
                        SA_LOG(L"Create Color RenderTarget ViewHeap success.", Info, DX12,
                               (L"\"%1\" [%2]", name, sceneRTViewHeap.Get()));
                    }

                    // Create RT Views (for each frame)
                    D3D12_CPU_DESCRIPTOR_HANDLE rtvHandle =
                        sceneRTViewHeap->GetCPUDescriptorHandleForHeapStart();
                    const UINT rtvOffset =
                        device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);

                    for (uint32_t i = 0; i < bufferingCount; ++i)
                    {
                        device->CreateRenderTargetView(swapchainImages[i].Get(), nullptr,
                                                       rtvHandle);
                ...
                // Depth Scene Texture /* 0005-I1 */
                {
                    ...
                    const HRESULT hrCreateDepthTexture = device->CreateCommittedResource(
                        &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_DEPTH_WRITE,
                        &sceneDepthClearValue, IID_PPV_ARGS(&sceneDepthTexture));
                    if (FAILED(hrCreateDepthTexture))
                    {
                        SA_LOG(L"Create Scene Depth Texture failed!", Error, DX12,
                               (L"Error Code: %1", hrCreateDepthTexture));
                        return EXIT_FAILURE;
                    }
                    else
                    {
                        ...
                        SA_LOG(L"Create Scene Depth Texture success.", Info, DX12,
                               (L"\"%1\" [%2]", name, sceneDepthTexture.Get()));
                    }
                }
                ...
                // Depth Scene RT View Heap /* 0006-I2 */
                {
                    ...
                    else
                    {
                        ...
                        SA_LOG(L"Create Depth ViewHeap success", Info, DX12,
                               (L"\"%1\" [%2]", name, sceneDepthRTViewHeap.Get()));
                    }
                    ...
                    device->CreateDepthStencilView(
                        sceneDepthTexture.Get(), nullptr,
                        sceneDepthRTViewHeap->GetCPUDescriptorHandleForHeapStart());
                }
            }
            ...
            if (true)
            {
                ...
                // Lit
                {
                    // RootSignature /* 0008-1-I */
                    {
                         ...
                         * Since there is no 'DescriptorSetLayout' in DX12, we have to manually
                        ...
                        const HRESULT hrSerRootSign =
                            D3D12SerializeVersionedRootSignature(&desc, &signature, &error);
                        if (FAILED(hrSerRootSign))
                        {
                            std::string errorStr(static_cast<char*>(error->GetBufferPointer()),
                                                 error->GetBufferSize());
                        ...
                        {
                            SA_LOG(L"Serialized Lit RootSignature success.", Info, DX12,
                                   signature.Get());
                        }

                        const HRESULT hrCreateRootSign = device->CreateRootSignature(
                            0, signature->GetBufferPointer(), signature->GetBufferSize(),
                            IID_PPV_ARGS(&litRootSign));
                        if (FAILED(hrCreateRootSign))
                        {
                            SA_LOG(L"Create Lit RootSignature failed!", Error, DX12,
                                   (L"Error Code: %1", hrCreateRootSign));
                            return EXIT_FAILURE;
                        }
                        else
                        {
                            SA_LOG(L"Create Lit RootSignature success.", Info, DX12,
                                   litRootSign.Get());
                        }
                    }
                    ...
                    // PipelineState
                    {
                        ...
                        const D3D12_BLEND_DESC blendState{
                            ...
                            .RenderTarget{rtBlend, rtBlend, rtBlend, rtBlend, rtBlend, rtBlend,
                                          rtBlend, rtBlend}
                        };
            ...
            if (true)
            {
                // PBR Sphere SRV View Heap
                {
                    ...
                    if (FAILED(hrCreateHeap))
                    {
                        SA_LOG(L"Create PBR Sphere SRV ViewHeap failed.", Error, DX12,
                               (L"Error code: %1", hrCreateHeap));
                        return EXIT_FAILURE;
                    }
                    else
                    {
                        ...
                        SA_LOG(L"Create PBR Sphere SRV ViewHeap success.", Info, DX12,
                               (L"\"%1\" [%2]", name, pbrSphereSRVHeap.Get()));
                    }
                }
                ...
                // Camera Buffers
                {
                    ...
                    for (uint32_t i = 0; i < bufferingCount; ++i)
                    {
                        ...
                        {
                            SA_LOG((L"Create Camera Buffer [%1] failed!", i), Error, DX12,
                        ...
                        else
                        {
                            ...
                            SA_LOG((L"Create Camera Buffer [%1] success", i), Info, DX12,
                                   (L"\"%1\" [%2]", name, cameraBuffers[i].Get()));
                        }
                    }
                }
                ...
                // Sphere Object Buffer
                {
                    ...
                    const HRESULT hrBufferCreated = device->CreateCommittedResource(
                        ...
                        IID_PPV_ARGS(&sphereObjectBuffer));
                    if (FAILED(hrBufferCreated))
                    {
                    ...
                    else
                    {
                        ...
                        SA_LOG(L"Create Sphere Object Buffer failed!", Info, DX12,
                               (L"\"%1\" [%2]", name, sphereObjectBuffer.Get()));
                    }

                    const SA::Mat4f transform = SA::Mat4f::MakeTranslation(spherePosition);
                    const bool bSubmitSuccess =
                        SubmitBufferToGPU(sphereObjectBuffer, desc.Width, &transform,
                ...
                // PointLights Buffer
                {
                    ...
                    const HRESULT hrBufferCreated = device->CreateCommittedResource(
                        &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON, nullptr,
                        IID_PPV_ARGS(&pointLightBuffer));
                    if (FAILED(hrBufferCreated))
                    {
                        SA_LOG(L"Create PointLights Buffer failed!", Error, DX12,
                               (L"Error code: %1", hrBufferCreated));
                        return EXIT_FAILURE;
                    }
                    else
                    {
                        ...
                        SA_LOG(L"Create PointLights Buffer success", Info, DX12,
                               (L"\"%1\" [%2]", name, pointLightBuffer.Get()));
                    }

                    std::array<PointLightUBO, pointLightNum> pointlightsUBO{
                        PointLightUBO{.position = SA::Vec3f(-0.25f, -1.0f, 0.0f),
                                      .intensity = 4.0f,
                                      .color = SA::Vec3f(1.0f, 1.0f, 0.0f),
                                      .radius = 3.0f},
                        PointLightUBO{.position = SA::Vec3f(1.75f,  2.0f,  1.0f),
                                      .intensity = 7.0f,
                    ...
                    const bool bSubmitSuccess =
                        SubmitBufferToGPU(pointLightBuffer, desc.Width, pointlightsUBO.data(),
                                          D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
                    if (!bSubmitSuccess)
                    {
                    ...
                    // Create View /* 0011-I-1 */
                    {
                        ...
                        device->CreateShaderResourceView(pointLightBuffer.Get(), &viewDesc,
                                                         cpuHandle);
                    }
                }
            }
            ...
            if (true)
            {
                ...
                // Meshes
                {
                    // Sphere
                    {
                        ...
                        const aiScene* scene = importer.ReadFile(
                            path, aiProcess_CalcTangentSpace | aiProcess_ConvertToLeftHanded);
                        if (!scene)
                        {
                        ...
                        // Position
                        {
                            ...
                            const HRESULT hrBufferCreated = device->CreateCommittedResource(
                                &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON,
                                nullptr, IID_PPV_ARGS(&sphereVertexBuffers[0]));
                            if (FAILED(hrBufferCreated))
                            {
                            ...
                            else
                            {
                                ...
                                SA_LOG(L"Create Sphere Vertex Position Buffer success.", Info, DX12,
                                       (L"\"%1\" [%2]", name, sphereVertexBuffers[0].Get()));
                            }
                            ...
                            const bool bSubmitSuccess = SubmitBufferToGPU(
                                sphereVertexBuffers[0], desc.Width, inMesh->mVertices,
                                D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
                            if (!bSubmitSuccess)
                            {
                                SA_LOG(L"Sphere Vertex Position Buffer submit failed!", Error,
                                       DX12);
                                return EXIT_FAILURE;
                            }
                        }
                        ...
                        // Normal
                        {
                            ...
                            const HRESULT hrBufferCreated = device->CreateCommittedResource(
                                &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON,
                                nullptr, IID_PPV_ARGS(&sphereVertexBuffers[1]));
                            if (FAILED(hrBufferCreated))
                            {
                            ...
                            else
                            {
                                ...
                                SA_LOG(L"Create Sphere Vertex Normal Buffer success.", Info, DX12,
                                       (L"\"%1\" [%2]", name, sphereVertexBuffers[1].Get()));
                            }
                            ...
                            const bool bSubmitSuccess = SubmitBufferToGPU(
                                sphereVertexBuffers[1], desc.Width, inMesh->mNormals,
                                D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
                            if (!bSubmitSuccess)
                            {
                        ...
                        // Tangent
                        {
                            ...
                            const HRESULT hrBufferCreated = device->CreateCommittedResource(
                                &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON,
                                nullptr, IID_PPV_ARGS(&sphereVertexBuffers[2]));
                            if (FAILED(hrBufferCreated))
                            {
                            ...
                            else
                            {
                                ...
                                SA_LOG(L"Create Sphere Vertex Tangent Buffer success.", Info, DX12,
                                       (L"\"%1\" [%2]", name, sphereVertexBuffers[2].Get()));
                            }
                            ...
                            const bool bSubmitSuccess = SubmitBufferToGPU(
                                sphereVertexBuffers[2], desc.Width, inMesh->mTangents,
                                D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
                            if (!bSubmitSuccess)
                            {
                        ...
                        // UV
                        {
                            ...
                            const HRESULT hrBufferCreated = device->CreateCommittedResource(
                                &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON,
                                nullptr, IID_PPV_ARGS(&sphereVertexBuffers[3]));
                            if (FAILED(hrBufferCreated))
                            {
                            ...
                            else
                            {
                                ...
                                SA_LOG(L"Create Sphere Vertex UV Buffer success.", Info, DX12,
                                       (L"\"%1\" [%2]", name, sphereVertexBuffers[3].Get()));
                            }
                            ...
                            {
                                uvs.push_back(SA::Vec2f{inMesh->mTextureCoords[0][i].x,
                                                        inMesh->mTextureCoords[0][i].y});
                            }

                            const bool bSubmitSuccess =
                                SubmitBufferToGPU(sphereVertexBuffers[3], desc.Width, uvs.data(),
                                                  D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
                            if (!bSubmitSuccess)
                            {
                        ...
                        // Index
                        {
                            ...
                            else
                            {
                                ...
                                SA_LOG(L"Create Sphere Index Buffer success.", Info, DX12,
                                       (L"\"%1\" [%2]", name, sphereIndexBuffer.Get()));
                            }
                            ...
                            {
                                indices[i * 3] =
                                    static_cast<uint16_t>(inMesh->mFaces[i].mIndices[0]);
                                indices[i * 3 + 1] =
                                    static_cast<uint16_t>(inMesh->mFaces[i].mIndices[1]);
                                indices[i * 3 + 2] =
                                    static_cast<uint16_t>(inMesh->mFaces[i].mIndices[2]);
                            }

                            const bool bSubmitSuccess =
                                SubmitBufferToGPU(sphereIndexBuffer, desc.Width, indices.data(),
                ...
                if (true)
                {
                    ...
                    // RustedIron2 PBR
                    {
                        ...
                        {
                            const char* path =
                                "Resources/Textures/RustedIron2/rustediron2_basecolor.png";

                            int width, height, channels;
                            char* inData = reinterpret_cast<char*>(
                                stbi_load(path, &width, &height, &channels, 4));
                            if (!inData)
                            {
                                SA_LOG((L"STBI Texture Loading {%1} failed", path), Error, STB,
                                       stbi_failure_reason());
                                return EXIT_FAILURE;
                            }
                            ...
                            GenerateMipMapsCPU(SA::Vec2ui{static_cast<uint32_t>(width),
                                                          static_cast<uint32_t>(height)},
                                               data, mipLevels, totalSize, mipExtents, channels);
                            ...
                            const HRESULT hrBufferCreated = device->CreateCommittedResource(
                                ...
                                nullptr, IID_PPV_ARGS(&rustedIron2AlbedoTexture));
                            if (FAILED(hrBufferCreated))
                            {
                            ...
                            else
                            {
                                ...
                                SA_LOG(L"Create RustedIron2 Albedo Texture success.", Info, DX12,
                                       (L"\"%1\" [%2]", name, rustedIron2AlbedoTexture.Get()));
                            }

                            const bool bSubmitSuccess =
                                SubmitTextureToGPU(rustedIron2AlbedoTexture, mipExtents, totalSize,
                                                   channels, data.data());
                            if (!bSubmitSuccess)
                            {
                            ...
                            // Create View /* 0011-I-2 */
                            {
                                ...
                                device->CreateShaderResourceView(rustedIron2AlbedoTexture.Get(),
                                                                 &viewDesc, cpuHandle);
                        ...
                        {
                            const char* path =
                                "Resources/Textures/RustedIron2/rustediron2_normal.png";
                            ...
                            const HRESULT hrBufferCreated = device->CreateCommittedResource(
                                &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST,
                                nullptr, IID_PPV_ARGS(&rustedIron2NormalTexture));
                            if (FAILED(hrBufferCreated))
                            {
                            ...
                            else
                            {
                                ...
                                SA_LOG(L"Create RustedIron2 Normal Texture success.", Info, DX12,
                                       (L"\"%1\" [%2]", name, rustedIron2NormalTexture.Get()));
                            }

                            const bool bSubmitSuccess =
                                SubmitTextureToGPU(rustedIron2NormalTexture, mipExtents, totalSize,
                                                   channels, data.data());
                            if (!bSubmitSuccess)
                            {
                            ...
                            // Create View /* 0011-I-3 */
                            {
                                ...
                                device->CreateShaderResourceView(rustedIron2NormalTexture.Get(),
                                                                 &viewDesc, cpuHandle);
                                cpuHandle.ptr += srvOffset;
                            }
                        }
                        ...
                        {
                            const char* path =
                                "Resources/Textures/RustedIron2/rustediron2_metallic.png";

                            int width, height, channels;
                            char* inData = reinterpret_cast<char*>(
                                stbi_load(path, &width, &height, &channels, 1));
                            if (!inData)
                            {
                                SA_LOG((L"STBI Texture Loading {%1} failed", path), Error, STB,
                                       stbi_failure_reason());
                            ...
                            const HRESULT hrBufferCreated = device->CreateCommittedResource(
                                &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST,
                                nullptr, IID_PPV_ARGS(&rustedIron2MetallicTexture));
                            if (FAILED(hrBufferCreated))
                            {
                            ...
                            else
                            {
                                ...
                                SA_LOG(L"Create RustedIron2 Metallic Texture success.", Info, DX12,
                                       (L"\"%1\" [%2]", name, rustedIron2MetallicTexture.Get()));
                            }

                            const bool bSubmitSuccess =
                                SubmitTextureToGPU(rustedIron2MetallicTexture, mipExtents,
                                                   totalSize, channels, data.data());
                            if (!bSubmitSuccess)
                            {
                            ...
                            // Create View /* 0011-I-4 */
                            {
                                ...
                                device->CreateShaderResourceView(rustedIron2MetallicTexture.Get(),
                                                                 &viewDesc, cpuHandle);
                                cpuHandle.ptr += srvOffset;
                            }
                        }
                        ...
                        {
                            const char* path =
                                "Resources/Textures/RustedIron2/rustediron2_roughness.png";
                            ...
                            const HRESULT hrBufferCreated = device->CreateCommittedResource(
                                &heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST,
                                nullptr, IID_PPV_ARGS(&rustedIron2RoughnessTexture));
                            if (FAILED(hrBufferCreated))
                            {
                            ...
                            else
                            {
                                ...
                                SA_LOG(L"Create RustedIron2 Roughness Texture success.", Info, DX12,
                                       (L"\"%1\" [%2]", name, rustedIron2RoughnessTexture.Get()));
                            }

                            const bool bSubmitSuccess =
                                SubmitTextureToGPU(rustedIron2RoughnessTexture, mipExtents,
                                                   totalSize, channels, data.data());
                            if (!bSubmitSuccess)
                            {
                                SA_LOG(L"RustedIron2 Roughness Texture submit failed!", Error,
                                       DX12);
                                return EXIT_FAILURE;
                            }
                            ...
                            // Create View /* 0011-I-5 */
                            {
                                ...
                                device->CreateShaderResourceView(rustedIron2RoughnessTexture.Get(),
                                                                 &viewDesc, cpuHandle);
                                cpuHandle.ptr += srvOffset;
                            }
                        }
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            if (accumulateTime >= fixedTime)
            {
                ...
                // Process input
                {
                    ...
                    {
                        dx += static_cast<float>(mouseX - oldMouseX) * fixedTime * cameraRotSpeed *
                              SA::Maths::DegToRad<float>;
                        dy += static_cast<float>(mouseY - oldMouseY) * fixedTime * cameraRotSpeed *
                              SA::Maths::DegToRad<float>;

                        oldMouseX = mouseX;
                        oldMouseY = mouseY;

                        dx = dx > SA::Maths::Pi<float>    ? dx - SA::Maths::Pi<float>
                             : dx < -SA::Maths::Pi<float> ? dx + SA::Maths::Pi<float>
                                                          : dx;
                        dy = dy > SA::Maths::Pi<float>    ? dy - SA::Maths::Pi<float>
                             : dy < -SA::Maths::Pi<float> ? dy + SA::Maths::Pi<float>
                                                          : dy;

                        cameraTr.rotation =
                            SA::Quatf(cos(dx), 0, sin(dx), 0) * SA::Quatf(cos(dy), sin(dy), 0, 0);
                    }
                }
            }
            ...
            // Render
            {
                ...
                auto cameraBuffer = cameraBuffers[swapchainFrameIndex];
                {
                    ...
                    const SA::Mat4f perspective = SA::Mat4f::MakePerspective(
                        cameraFOV, float(windowSize.x) / float(windowSize.y), cameraNear,
                        cameraFar);
                ...
                // Register Commands /* 0004-U */
                {
                     ...
                     */
                    {
                        ...
                        // Bind & Clear /* 0006-U1 */
                        {
                            ...
                            D3D12_CPU_DESCRIPTOR_HANDLE rtvHandle =
                                sceneRTViewHeap->GetCPUDescriptorHandleForHeapStart();
                            const UINT rtvOffset = device->GetDescriptorHandleIncrementSize(
                            ...
                            cmd->ClearDepthStencilView(dsvHandle, D3D12_CLEAR_FLAG_DEPTH,
                                                       sceneDepthClearValue.DepthStencil.Depth,
                    ...
                    // Lit Pipeline
                    {
                        ...
                        /**
                         * 0011-U-2
                         * DirectX12 doesn't have DescriptorSet: manually bind each entry of the
                         * RootSignature.
                         */
                ...
                if (true)
                {
                    ...
                    const HRESULT hrFenceSignal =
                        graphicsQueue->Signal(swapchainFence.Get(), currFenceValue);
                    if (FAILED(hrFenceSignal))
                    {
                        SA_LOG(L"Swapchain Fence Signal failed", Error, DX12,
                               (L"Error code: %1", hrFenceSignal));
                        return EXIT_FAILURE;
                    }
                }
            }
        ...
        // Renderer
        {
            ...
            // Resources /* 0010-D */
            {
                // Textures
                {
                    // RustedIron 2
                    {
                        SA_LOG(L"Destroying RustedIron2 Roughness Texture...", Info, DX12,
                               rustedIron2RoughnessTexture.Get());
                        rustedIron2RoughnessTexture = nullptr;

                        SA_LOG(L"Destroying RustedIron2 Metallic Texture...", Info, DX12,
                               rustedIron2MetallicTexture.Get());
                        rustedIron2MetallicTexture = nullptr;

                        SA_LOG(L"Destroying RustedIron2 Normal Texture...", Info, DX12,
                               rustedIron2NormalTexture.Get());
                        rustedIron2NormalTexture = nullptr;

                        SA_LOG(L"Destroying RustedIron2 Albedo Texture...", Info, DX12,
                               rustedIron2AlbedoTexture.Get());
                ...
                // Meshes
                {
                    // Sphere
                    {
                        SA_LOG(L"Destroying Sphere Index Buffer...", Info, DX12,
                               sphereIndexBuffer.Get());
                        sphereIndexBuffer = nullptr;
                        sphereIndexBufferView = D3D12_INDEX_BUFFER_VIEW{};

                        SA_LOG(L"Destroying Sphere Vertex Position Buffer...", Info, DX12,
                               sphereVertexBuffers[0].Get());
                        sphereVertexBuffers[0] = nullptr;
                        sphereVertexBufferViews[0] = D3D12_VERTEX_BUFFER_VIEW{};

                        SA_LOG(L"Destroying Sphere Normal Position Buffer...", Info, DX12,
                               sphereVertexBuffers[1].Get());
                        sphereVertexBuffers[1] = nullptr;
                        sphereVertexBufferViews[1] = D3D12_VERTEX_BUFFER_VIEW{};

                        SA_LOG(L"Destroying Sphere Tangent Position Buffer...", Info, DX12,
                               sphereVertexBuffers[2].Get());
                        sphereVertexBuffers[2] = nullptr;
                        sphereVertexBufferViews[2] = D3D12_VERTEX_BUFFER_VIEW{};

                        SA_LOG(L"Destroying Sphere UV Position Buffer...", Info, DX12,
                               sphereVertexBuffers[3].Get());
            ...
            // Scene Objects /* 0009-D */
            {
                // Camera Buffer
                {
                    for (uint32_t i = 0; i < bufferingCount; ++i)
                    {
                        SA_LOG((L"Destroying Camera Buffer [%1]...", i), Info, DX12,
                               cameraBuffers[i].Get());
                ...
                // Sphere Object Buffer
                {
                    SA_LOG(L"Destroying Sphere Object Buffer...", Info, DX12,
                           sphereObjectBuffer.Get());
                ...
                // PBR Sphere ViewHeap
                {
                    SA_LOG(L"Destroying PBR Sphere SRV ViewHeap...", Info, DX12,
                           pbrSphereSRVHeap.Get());
            ...
            // Scene Resources /* 0005-D - 0006-D */
            {
                ...
                SA_LOG(L"Destroying Scene Depth RT ViewHeap...", Info, DX12,
                       sceneDepthRTViewHeap.Get());
            ...
            // Commands /* 0004-D */
            {
                ...
                for (uint32_t i = 0; i < bufferingCount; ++i)
                {
                    SA_LOG((L"Destroying Command Allocator [%1]...", i), Info, DX12,
                           cmdAllocs[i].Get());
                    cmdAllocs[i] = nullptr;
                }
            }
            ...
            // Swapchain /* 0003-D */
            {
                // Synchronization
                {
                    ...
                    SA_LOG(L"Destroy Swapchain Fence Event success", Info, DX12,
                           swapchainFenceEvent);
                ...
                for (uint32_t i = 0; i < bufferingCount; ++i)
                {
                    SA_LOG((L"Destroying Swapchain image [%1]...", i), Info, DX12,
                           swapchainImages[i].Get());
